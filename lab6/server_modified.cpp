...
uint8_t stringToType(const std::string &value)
{
    ...
    if (value == "MSG_ACK") return MSG_ACK;
...
std::mt19937 &threadRng()
{
    static thread_local std::mt19937 rng(
        ...
        static_cast<unsigned int>(reinterpret_cast<uintptr_t>(pthread_self())));
    return rng;
}
...
{
    if (simConfig.delay_ms > 0)
    {
        usleep(simConfig.delay_ms * 1000);
        std::cout << "[Transport][SIM] DELAY applied: " << simConfig.delay_ms << " ms" << std::endl;
    }

    if (simConfig.drop_rate > 0.0 && randomUnit() < simConfig.drop_rate)
    {
        std::cout << "[Transport][SIM] DROP (id=" << msg.msg_id << ", rate=" << simConfig.drop_rate << ")" << std::endl;
        return false;
    }

    if (simConfig.corrupt_rate > 0.0 && randomUnit() < simConfig.corrupt_rate)
    {
        size_t payloadLength = strnlen(msg.payload, MAX_PAYLOAD);
        if (payloadLength > 0)
        {
            std::uniform_int_distribution<size_t> indexDist(0, payloadLength - 1);
            size_t pos = indexDist(threadRng());
...
bool isReliableInboundMessage(uint8_t type)
{
    return type == MSG_TEXT || type == MSG_PRIVATE || type == MSG_PING;
}

bool registerOrDetectDuplicate(int sock, uint32_t msgId)
{
    ...
    for (Client &client : clients)
    {
        ...
        client.last_ids[client.last_id_pos % 32] = msgId;
...
void sendAck(int sock, uint32_t msgId)
{
    std::cout << "[Transport][ACK] send MSG_ACK (id=" << msgId << ")" << std::endl;
    sendMessageEx(sock, MSG_ACK, "SERVER", "", "ACK", "delivery acknowledgement", msgId, std::time(nullptr));
}
...
void handleClientMessages(int clientSock, const std::string &nickname)
{
    while (true)
    {
        ...
        if (isReliableInboundMessage(msg.type))
        {
            if (registerOrDetectDuplicate(clientSock, msg.msg_id))
            {
                std::cout << "[Application][DEDUP] duplicate ignored (id=" << msg.msg_id << ")" << std::endl;
                sendAck(clientSock, msg.msg_id);
                continue;
            }
            std::cout << "[Application][ACK] process " << typeToString(msg.type) << " (id=" << msg.msg_id << ")" << std::endl;
        }
        else
        {
        ...
        else if (msg.type == MSG_PING)
        {
            ...
            sendMessageEx(clientSock, MSG_PONG, "SERVER", "", "[SERVER]: PONG", "ping response", msg.msg_id, std::time(nullptr));
            sendAck(clientSock, msg.msg_id);
        }
...
void parseSimulationArgs(int argc, char **argv)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        try
        {
            ...
            {
                simConfig.delay_ms = std::max(0, std::stoi(arg.substr(8)));
            }
            else if (arg.rfind("--drop=", 0) == 0)
            {
                simConfig.drop_rate = std::stod(arg.substr(7));
                if (simConfig.drop_rate < 0.0) simConfig.drop_rate = 0.0;
                if (simConfig.drop_rate > 1.0) simConfig.drop_rate = 1.0;
            }
            else if (arg.rfind("--corrupt=", 0) == 0)
            {
                simConfig.corrupt_rate = std::stod(arg.substr(10));
                if (simConfig.corrupt_rate < 0.0) simConfig.corrupt_rate = 0.0;
                if (simConfig.corrupt_rate > 1.0) simConfig.corrupt_rate = 1.0;
            }
            else
            {
                std::cout << "Unknown parameter ignored: " << arg << std::endl;
            }
        }
        catch (...)
        {
            std::cout << "Invalid parameter ignored: " << arg << std::endl;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    std::cout << "[Transport][SIM] config delay=" << simConfig.delay_ms
              << "ms drop=" << simConfig.drop_rate
              << " corrupt=" << simConfig.corrupt_rate << std::endl;
